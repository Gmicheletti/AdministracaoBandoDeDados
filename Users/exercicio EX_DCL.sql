...
------------- NUMERO 1 --------------
-- A empresa de tecnologia TINEWTON precisa de um DBA para fazer a gestão de controle de seus usuários.
...
SELECT user(); -- comando1
SELECT user, host from mysql.user; -- comando2
SELECT * from mysql.user; -- comando3
DROP USER 'maria'@'localhost'; -- comando4


------------- NUMERO 2 --------------
-- Agora teremos apenas os usuarios do sistema cadastrados, então podemos começar a cadastrar os colaboradores.
-- Crie os seguintes usuarios:
-- JoanaASCII - 123321 #Local
-- KarenMouse - sem senha #Local
-- TioTeclas - 131312 #remoto
-- Teclaudio - 369099 #remoto
-- RonanAsus - sem senha #Local
-- MarcusTeras - 451212 #Local

CREATE USER
    'JoanaASCII'@'localhost' IDENTIFIED BY '123321',
...
ALTER USER
    'KarenMouse'@'localhost' IDENTIFIED BY '123456',
...
-- O usuario Teclaudio deve receber deve receber um novo padrão de uso, onde deve-se inserir um historico de utilização de senha onde não se pode usar a mesma senha antes de alterá-la 3 vezes com senhas diferentes

ALTER USER 'KarenMouse'@'localhost' IDENTIFIED BY '121212';
ALTER USER 'KarenMouse'@'localhost' PASSWORD EXPIRE NEVER;

ALTER USER 'TioTeclas'@'%' IDENTIFIED BY '1234';
ALTER USER 'TioTeclas'@'%' PASSWORD EXPIRE INTERVAL 90 DAY;

ALTER USER 'RonanAsus'@'localhost' IDENTIFIED BY 'R232323';
ALTER USER 'RonanAsus'@'localhost' PASSWORD REQUIRE CURRENT;
